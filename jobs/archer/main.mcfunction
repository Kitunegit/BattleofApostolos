...
#矢処理
    #矢常時実行
        ...
        execute if score @s skill_2_cool matches 300.. at @e[type=arrow] run particle dust 0.71 0.831 0 0.75 ~ ~ ~ 0.15 0.15 0.15 2 2
    #着弾処理
        execute as @e[type=arrow] at @s if entity @s[nbt={life:1s}] run particle dust 0 1 0.45108 2 ~ ~ ~
        execute if score @s skill_2_cool matches 300.. as @e[type=arrow] at @s if entity @s[nbt={life:1s}] run effect give @e[distance=..10,tag=player] glowing 20 1 true
        execute if score @s skill_2_cool matches 300.. as @e[type=arrow] at @s if entity @s[nbt={life:1s}] run particle flash ~ ~ ~ 10 10 10 0.5 80
