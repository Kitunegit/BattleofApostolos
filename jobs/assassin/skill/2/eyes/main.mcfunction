#無条件処理
    ...
    execute as @e[type=marker,tag=black_eyes] at @s positioned ^ ^ ^2 if block ~ ~ ~ air run tp @s ^ ^ ^2
...
#ブロックに当たったとき
    execute as @e[type=marker,tag=black_eyes] at @s positioned ^ ^ ^2 unless block ~ ~ ~ air run particle dust 0 0 0 4 ^ ^ ^-3 2 2 2 1 120
    execute as @e[type=marker,tag=black_eyes] at @s positioned ^ ^ ^2 unless block ~ ~ ~ air run playsound item.glow_ink_sac.use player @a[distance=..10] ~ ~ ~ 15 1
