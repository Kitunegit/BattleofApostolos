#スキル1処理
    #発動処理
        execute as @s[nbt={SelectedItem:{tag:{CustomModelData:6},id:"minecraft:carrot_on_a_stick"}},scores={assassin_special_cool=..0,r_click=1..}] at @s positioned ^ ^ ^8 at @e[distance=..7.99,tag=player] rotated ~ 0 if block ^ ^ ^-1 air run function pvp_data:pvpfunctions/jobs/assassin/special/skill_start
