    ...
    execute if entity @e[distance=..3.9,limit=3,tag=player] as @s[scores={jobscore=3,special_count=1}] at @s positioned ^ ^ ^4 as @e[distance=..3.9,limit=3,tag=player] run tag @s add knight_special_target
    execute as @s[scores={jobscore=3,special_count=1}] unless entity @e[tag=knight_special_target] run playsound entity.bat.death player @s ~ ~ ~ 0.6 1
