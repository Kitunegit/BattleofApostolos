#> pvp_data:pvpfunctions/jobs/guardian/shield/break_strong
#
...
# 演出
    ...
    execute at @s run playsound minecraft:item.shield.break player @a[distance=..15] ~ ~ ~ 1.5 0.4
    execute at @s run playsound minecraft:item.shield.break player @a[distance=..15] ~ ~ ~ 1.5 0.4
    execute at @s run playsound minecraft:entity.evoker_fangs.attack player @a[distance=..15] ~ ~ ~ 1 1.5
# ブレイク状態にする(デバフ)
    function pvp_data:pvpfunctions/systems/status/effect/apply {name:generic.speed,depth:-1,duration:120,overwrite:false}
    function pvp_data:pvpfunctions/systems/status/effect/apply {name:generic.defense,depth:-300,duration:120,overwrite:false}
# 修復までのCT
