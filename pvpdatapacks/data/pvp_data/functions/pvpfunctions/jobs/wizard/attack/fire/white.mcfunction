#> pvp_data:pvpfunctions/jobs/wizard/attack/fire/white
#
# 魔導士白魔法弾
#
# @internal
...
# 魔法陣を出して、向きを合わせる
    $execute at @s positioned ~ ~100.4 ~ run summon item_display $(pos) {item:{id:"minecraft:quartz",Count:1b,tag:{CustomModelData:1}},Tags:[MagicCircle],brightness:{sky:15,block:15}}
