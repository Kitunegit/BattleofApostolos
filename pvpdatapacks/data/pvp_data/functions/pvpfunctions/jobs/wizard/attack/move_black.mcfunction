    ...
    execute if score @s wizard_normal_count matches 9.. at @s run function pvp_data:pvpfunctions/systems/job_system/damage/apply {damage:{amount: 12f, type: "pvp_data:wizard/wizard_normal_magic"},knockback:{horizontal: 1.2d,vertical:1.5d}, target: "@e[tag=player,disntance=..2.7,tag=!wizard.magic_fire,type=!item_display]"}
