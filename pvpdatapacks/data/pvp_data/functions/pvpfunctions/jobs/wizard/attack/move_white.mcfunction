    ...
    execute at @s positioned ~-0.5 ~ ~-0.5 if entity @e[dx=0] if score @s wizard_normal_count matches 2.. as @a[tag=wizard.magic_fire] run function pvp_data:pvpfunctions/systems/job_system/damage/apply {damage:{amount: 2f, type: "pvp_data:wizard/wizard_normal_magic"},knockback:{horizontal: 0d,vertical:0d}, target: "@e[tag=player,dx=0]"}
