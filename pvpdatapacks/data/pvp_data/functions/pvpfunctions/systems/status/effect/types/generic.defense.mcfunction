...
# スコアへ代入
    ...
    # 現在の値へ代入
        scoreboard players operation $modifier_depth system.status.effect.calc_modifier = @s generic.defense.effect_depth

        scoreboard players operation $modifier_depth system.status.effect.calc_modifier += @s generic.defense.base

        scoreboard players operation @s generic.defense = $modifier_depth system.status.effect.calc_modifier
