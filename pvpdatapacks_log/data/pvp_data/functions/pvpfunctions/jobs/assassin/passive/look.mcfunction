...
# 発見値上昇
    scoreboard players add @s[scores={passive_count=..1200}] passive_count 4
# 条件到達時、Reset関数に飛ばす
    execute if score @s passive_count matches 1200.. run function pvp_data:pvpfunctions/jobs/assassin/passive/reset
# 演出(仮)
